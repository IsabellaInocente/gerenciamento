...
import com.hospedagem.services.TarefaService;
...
public class TarefaController {
	...
	@GetMapping("/")
	public ResponseEntity<List<Tarefa>> buscaTarefaControl(){
		List<Tarefa> tarefas = tarefaService.buscaTarefa();
	...
	public ResponseEntity<Tarefa> alteraTarefaControl(@PathVariable Long id, @RequestBody Tarefa tarefa){
		Tarefa mudarTarefa = tarefaService.mudarTarefa(id, tarefa);
		if(mudarTarefa != null) {
			return ResponseEntity.ok(mudarTarefa);
		}
		else {
			return ResponseEntity.notFound().build();
		}
	...
	public ResponseEntity<String> apagarTarefaControl(@PathVariable Long id){
		...
		if(apagar) {
			return ResponseEntity.ok().body("a tarefa foi add");
		}
		else { 
			return ResponseEntity.notFound().build();
		}
